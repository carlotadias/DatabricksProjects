...
**import the PI/enterprise CA into the cluster trust store** (via a cluster init
script) and keep `verify_tls=true`. Note `verify_tls=false` fixes only *certificate*
errors — it does **not** help if DNS can't resolve the PI FQDN from the cluster.
