...
HTTP_TIMEOUT = 30
# --- WebID resolution (parallel) ---------------------------------------------
# Resolution runs across the cluster: CLUSTER_CORES partitions x RESOLVE_THREADS
# threads. It replaced a serial driver loop that did ~18 req/s -- 15,000 tags took
# over 14 minutes against SSE's PI while every task slot sat idle.
#
# Why 4 and not 32: PI's general limit is 1,000 requests/s per client IP, but
# RateLimitSearchMaxRequests is only 50/s, and the admin guide does not state
# whether a path-based GET /points?path= counts as a search. If it does, more
# threads produce HTTP 429 instead of speed. 429 is retried with backoff and the
# retry count is printed after resolution -- raise this only while it stays at 0.
RESOLVE_THREADS = 4
RESOLVE_MAX_RETRIES = 4

# NOTE: there is no early give-up any more. The old GIVE_UP_AFTER aborted a serial
# loop after N consecutive failures with nothing resolved, to avoid waiting out
# 15,000 doomed requests on a wrong PI_SERVER. In parallel the whole list finishes
# in seconds, so the guard bought nothing and only risked truncating a partly
# working tag list. Failures are still grouped by cause and printed in full.
...
if WEB_IDS:
    ...
    # but it is the only POST here, and a PI Web API behind a read-only gateway refuses it
    # ("403 Site access denied for .../piwebapi/batch") even though its sub-requests are
    # GETs; PI cannot inspect the body to know that. GET /points always works where reads
    # work, so it is now the default and Batch is opportunistic.
    #
    # PI_SERVER must be the DATA ARCHIVE name with PI's exact casing. Get it from
    # GET /dataservers -> Items[].Name; it is NOT the Web API hostname, and NOT an AF
    # database name.
    WEB_IDS, _bad, _errs = [], [], {}

    # Fast path: one POST for everything. Silently skipped if the server refuses POST.
    _batched = False
    if len(TAGS) > 1:
        try:
            ...
            _batched = True
            print(f"resolved {len(WEB_IDS)}/{len(TAGS)} WebIDs via POST /batch "
                  f"on {PI_SERVER}")
        except Exception as _be:
            print(f"POST /batch unavailable ({_cause(_be)})")
            print(f"  -> using GET /points instead: {len(TAGS)} requests rather than 1,"
                  f" spread across the cluster.")
            print(f"     Worth asking the PI admin to enable the Batch controller — it is"
                  f" the same\n     lookups in one round trip.")
            WEB_IDS, _bad = [], []

    # Default path: one GET per tag, spread ACROSS THE CLUSTER.
    #
    # This used to be a serial for-loop on the driver. Measured against SSE's PI:
    # 15,000 tags took over 14 minutes, which is ~18 requests/second, while every
    # task slot on the cluster sat idle. PI's own limit is 1,000/s per client IP,
    # so the serial loop used under 2% of the allowed rate -- the cost was purely
    # round-trip latency, one request at a time.
    #
    # Concurrency is kept MODEST on purpose. RateLimitSearchMaxRequests defaults to
    # 50/s per IP (20x tighter than the general limit) and the admin guide does not
    # say whether a path-based GET /points?path= counts as a search. If it does,
    # more threads earn HTTP 429 rather than throughput. So 429 is retried with
    # backoff and the retry count is printed: zero means RESOLVE_THREADS can go up,
    # non-zero means it should come down.
    if not _batched:
        _parts = max(1, min(CLUSTER_CORES, len(TAGS)))
        _inflight = _parts * RESOLVE_THREADS
        print(f"     resolving {len(TAGS):,} tags: {_parts} partitions x "
              f"{RESOLVE_THREADS} threads = {_inflight} concurrent GETs")

        # Plain values only: a requests.Session cannot be pickled, so each
        # partition builds its own.
        _rcfg = sc.broadcast({"base": BASE, "server": PI_SERVER, "auth": AUTH,
                              "verify_tls": VERIFY_TLS, "timeout": HTTP_TIMEOUT,
                              "threads": RESOLVE_THREADS,
                              "retries": RESOLVE_MAX_RETRIES})

        def _resolve_part(_tag_iter):
            """Resolve one partition's tags on an executor.

            Yields (tag, web_id, error, retries). One HTTP session per partition so
            TLS is negotiated once instead of per tag, and the connection pool is
            sized to the thread count -- without that, requests serialises on the
            pool and the extra threads buy nothing.
            """
            import time as _t
            import requests
            ...
            _sx = requests.Session()
            _ad = HTTPAdapter(pool_connections=_c["threads"],
                              pool_maxsize=_c["threads"])
            _sx.mount("https://", _ad)
            _sx.mount("http://", _ad)
            _a = _c["auth"]
            if _a.get("bearer"):
                _sx.headers["Authorization"] = f"Bearer {_a['bearer']}"
            elif _a.get("basic_user"):
                _sx.auth = (_a["basic_user"], _a.get("basic_password"))
            _sx.verify = _c["verify_tls"]
            _url = f"{_c['base']}/points"

            def _one(_tag):
                _p = {"path": rf"\\{_c['server']}\{_tag}"}
                _n = 0
                for _att in range(_c["retries"] + 1):
                    try:
                        _r = _sx.get(_url, params=_p, timeout=_c["timeout"])
                        # 429 means WE are the problem, not the tag.
                        if _r.status_code == 429:
                            if _att == _c["retries"]:
                                return (_tag, None, "429 rate limited (retries exhausted)", _n)
                            _w = float(_r.headers.get("Retry-After") or 0.5 * (2 ** _att))
                            _t.sleep(min(_w, 10.0)); _n += 1; continue
                        if 500 <= _r.status_code < 600:
                            if _att == _c["retries"]:
                                return (_tag, None, f"{_r.status_code} server error", _n)
                            _t.sleep(min(0.5 * (2 ** _att), 10.0)); _n += 1; continue
                        if _r.status_code != 200:
                            # 404 (not on this archive) and 403 (lookup refused) are
                            # final: retrying cannot change them.
                            return (_tag, None,
                                    f"{_r.status_code} {(_r.text or '')[:110]}".replace("\n", " "), _n)
                        _w2 = _r.json().get("WebId")
                        return (_tag, _w2, None if _w2 else "200 but no WebId", _n)
                    except Exception as _pe2:
                        if _att == _c["retries"]:
                            return (_tag, None, f"{type(_pe2).__name__}: {str(_pe2)[:100]}", _n)
                        _t.sleep(min(0.5 * (2 ** _att), 10.0)); _n += 1
                return (_tag, None, "unreachable", _n)

            with ThreadPoolExecutor(max_workers=_c["threads"]) as _pool:
                for _row in _pool.map(_one, _tg):
                    yield _row

        _rt0 = time.perf_counter()
        _rows = sc.parallelize(TAGS, _parts).mapPartitions(_resolve_part).collect()
        _rsecs = time.perf_counter() - _rt0

        # Keep WEB_IDS in TAGS order, not partition-completion order, so a run is
        # comparable with the serial version it replaces.
        _map = {t: (w, e) for t, w, e, _ in _rows}
        for t in TAGS:
            _w, _e = _map.get(t, (None, "missing from results"))
            if _w:
                WEB_IDS.append(_w)
            else:
                _bad.append(t)
                # Group by cause: if everything fails it is one shared reason
                # (wrong PI_SERVER casing, tags not on this archive), not N bad
                # names. The per-request URL differs per tag, so strip it first.
                _k = re.sub(r"\s*for url:.*$", "", str(_e)).strip()[:120]
                _errs[_k] = _errs.get(_k, 0) + 1

        _rtry = sum(r for *_, r in _rows)
        print(f"     resolved {len(WEB_IDS)}/{len(TAGS)} WebIDs via GET /points")
        print(f"     {_rsecs:,.1f}s at {len(TAGS)/max(_rsecs,1e-9):,.0f} req/s"
              f"  ({_rsecs/max(1,len(TAGS))*1000:.0f} ms/tag effective)")
        if _rtry:
            print(f"     {_rtry} retries (429/5xx) — lower RESOLVE_THREADS if large")
        for _k, _n in sorted(_errs.items(), key=lambda x: -x[1]):
            print(f"       {_n:>5}x  {_k}")
        if _errs and not WEB_IDS:
            print(f"     ⚠️ NOTHING resolved. Confirm the archive name and casing with:")
            print(f"          r = _sess.get(f'{{BASE}}/dataservers'); "
                  f"[i['Name'] for i in r.json()['Items']]")
            print(f"        PI_SERVER is currently '{PI_SERVER}'. Then check one tag by hand:")
            print(f"          _sess.get(f'{{BASE}}/points', "
                  f"params={{'path': r'\\\\{PI_SERVER}\\<a real tag>'}})")
    if _bad:
        print(f"   ⚠️ {len(_bad)} did NOT resolve. All of them:")
        for _i in range(0, len(_bad), 4):
            print("        " + "  ".join(_bad[_i:_i + 4]))
...
for _m in ("value", "recorded", "interpolated"):
    try:
        ...
        _smoke.append({"mode": _m, "rows": None, "columns": None, "status": _cause(e)})
        _ok_all = False
        print(f"  {_m:<13} FAILED: {_cause(e)}")
...
# The connector puts one &webId=<id> per tag in the QUERY STRING, so a big chunk with
# long WebIDs can exceed IIS's limits (maxQueryString 2048 / maxUrl 4096 by default) and
# PI rejects the request outright. AF *attribute* WebIDs are long — 130-200 chars — so
# this bites well before PI's own item limits do.
_idlen = max(len(w) for w in WEB_IDS)
for _c in sorted(set(HIST_CHUNKS + CHUNKS)):
    _qs = _c * (_idlen + 8)
    if _qs > 4096:
        print(f"⚠️  chunk={_c}: ~{_qs:,}-char query string (longest WebID {_idlen} chars). "
              f"IIS caps\n    maxUrl at 4096 by default, so this MAY be rejected — a chunk "
              f"that fails while a\n    smaller one succeeds is the signature. If so, lower "
              f"the chunk or ask them to raise\n    maxQueryString/maxUrl in "
              f"PI Web API's web.config.")
print()
    ...
    for _chunk in HIST_CHUNKS:
        for _conc in HIST_CONC:
            ...
            try:
                ...
                print(f"  {_hours}h chunk={_chunk:<4} conc={_conc:<2} FAILED: {_cause(e)}")
