...
if WEB_IDS:
    ...
    # Default path: one GET per tag. Slower, but it works wherever reads work.
    if not _batched:
        for t in TAGS:
            try:
                ...
                _bad.append(t)
                # Group the reason: if everything fails it is one shared cause (wrong
                # PI_SERVER casing, tags not on this archive), not N bad tag names.
                # Drop the per-request URL before grouping: it differs for every tag,
                # so one shared cause would otherwise be reported once per tag prefix.
                _k = re.sub(r"\s*for url:.*$", "", _cause(_pe)).strip()[:120]
                _errs[_k] = _errs.get(_k, 0) + 1
                if len(_bad) >= GIVE_UP_AFTER and not WEB_IDS:
                    print(f"     stopping after {len(_bad)} failures with nothing"
                          f" resolved — raise GIVE_UP_AFTER to keep trying")
                    break
