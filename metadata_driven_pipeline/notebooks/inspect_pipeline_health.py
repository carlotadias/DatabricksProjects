...
bronze_tables = [
    ...
    if row.tableName.startswith("bronze_") and not row.tableName.startswith("bronze_agent_call") and not row.tableName.startswith("bronze_order_ful")
]
bronze_table_ref = f"{catalog}.{bronze_schema}.{bronze_tables[0]}" if bronze_tables else None
print(f"Using event log from: {bronze_table_ref}")
